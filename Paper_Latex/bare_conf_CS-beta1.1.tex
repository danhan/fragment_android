...
\begin{abstract}
  ...
  Labeled LDA (Latent Dirichlet Allocation) on the labeled data to
  infer their topics. 
  ...
  time for the bugs eported by each of the vendors, we categorized the
  topics into three types: \textit{Common Troubled Topics},
  % ES: I propose to make "Common Troubled Topics" into "common problems"
  \textit{Common Improved Topics} 
  % ES: I propose to make "Common Impoved Topics" into "common desired
  % improvements  
  and \textit{Unique Topics}. The \textit{Common Troubled Topics} show
  that there is no correlation between the troubled features of
  Android and Android evolution. The \textit{Common Improved Topics}
  show that some features within the same vendors have portability
  issues across their multiple devices. The existence of
  \textit{Unique Topics} shows that different vendors have specific bug
...
operating system. Second, vendors offer device-specific Andorid
versions with custom user interfaces. Finally, carriers also offer
...
consistently across devices and software.  Furthermore, it tends to
prevent some Android users from experiencing new features, when these
features are slow to appear on their chosen devices. These problems
may cause clients to lose confidence in the Android platform and cost
the brand market share. Given its potential impact, Android
fragmentation is the topic of much discussion and
controversy. However, there has been little systematic evidence on how
exactly it may impact consumer opinon.
...
Our objective in this study is to search for evidence of the Android
fragmentation in the bug reports submitted by users of Android devices
from different vendors. To that end, we applied topic analysis on the
Android bug reports. 
...
software engineering, including Latent Dirichlet Allocation (LDA)
\cite{Marcus04aninformation}, Latent Semantic Indexing (LSI)
\cite{Asuncion:2010}, and Labeled Latent Dirichlet Allocation
(Labeled-LDA) \cite{Linstead:2009}. We applied the
labeled-LDA\cite{labeledlda} topic-analysis method on the bug reports
of different vendors, and identified several recurring topics in these
bugs. Next, we examined the bug topics to gain some intuition on the
...
This paper makes two important contributions. First, it provides
specific evidence on Android features that contribute to both software
and hardware fragmentation. Second, and equally importantly, it lays
out a methodology, which can be used to examine fragmentation across
Android manufacturers, beyond the two we have already studied. 

The rest of the paper is organized as follows. Section 2 reviews the
background of our work and Section 3 discusses related work. Section 4
introduces our methodology, which is applied to our data set in
Section 5. Section 6 compares and evaluates the topic models generated
by LDA and labeled-LDA. The paper concludes with a discussion of two
research questions, threats to validity, conclusion and future work in
Sections 7, 8 and 9 correspondingly.
