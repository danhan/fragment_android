...
\begin{abstract}
%TODO 1 edit according to the review
...
%worsening.
The fragmentation of Android ecosystem causes portability and compatibility 
issues with entire Android platform,
which disappoints users, raises the workload for developers, and delays deployment. 
Many existing studies on fragmentation roughly demonstrate basic statistics, without
specific and concrete evidence about the existence of fragmentation.
%without specific information about where the fragmentation exists.
 In this study, we search for evidence of fragmentation in
...
%TODO add the citations mentioned by reviewer one.
Fragmentation tends to delay some users from updating their applications
and Android operating system until their specific devices are fully supported.
These problems
may cause users to lose confidence in the Android platform and damage
the brand market-share. Given its potential impact, Android
fragmentation is the topic of much discussion and
controversy. However, there has been little empirical evidence on where the fragmentation exists \cite{androidfragmentation}, \cite{uglyhead}, \cite{boonorbane}.
...
(LDA) \cite{Asuncion:2010}, \cite{Linstead:2009}, Latent Semantic Indexing
...
%TODO 5 edit background of Labeled LDA
With the multi-lables assigned manually for each document, Labeled-LDA can find out which parts of the document correspond to each label,
...
the evolution of topics within software projects. Hindle et al. \cite{hindle-icsm-2012} investigated 
whether the topics extracted by LDA make sense to practitioners.
...
and Motorola,
%TODO 4 edit according to the review
...
%TODO reviewer one argued about this.
The hyper-parameters $\alpha$ and $\beta$ of Labeled-LDA are both 0.01.

Our topic analysis is based on these results. 
To visualize the association of a label (an extracted
Labeled-LDA topic) to bug reports over time (we call it relevance and a larger relecance means 
a strong association of a label to a bug reports),
we grouped
all the bug reports by month, from 2009 to 2011, based on their opened
date for each of the two vendors.
%TODO 3 edit according to the review one
In order to elimilate other factors that might impact our analysis, such as the different number of bugs in 
each vendor,  
we then computed the average relevance values of bug
reports to this label in each month \cite{hindle-icsm-2012}. 
...
In order to compare the performance between LDA and Labeled-LDA, 
we applied LDA to the same processed bug reports of HTC and Motorola
but without our manual labels. 

Applying LDA had one complication, LDA requires an input, $n$ that determines the number of topics that LDA is supposed to extract. 
...
\cite{Thomas:2011}, \cite{Hindle2011}. 

We applied the same process to the bug reports of Motorola and we chose
the number of topics to be $n = 30$. The hyper-parameters $\alpha$ and $\beta$ for LDA are also both 0.01. 
...
% Jaccard equation is not that important. We can delete it if there is no space:(
\begin{equation}
...
study if each of the two distributions match. The results of comparison is discussed in \ref{sec:comparinglda}.
...
bug repository.
%TODO edited accroding to review
In addition, the bug reports, as one of the ways to retrieve evidence of fragmentation, 
might not comprehensively reflect all the existence of fragmentation.
...
Our findings can be used to make project
dashboards, process investigation and feature analysis; 
...
\begin{table*}[!htb]
\renewcommand{\arraystretch}{1.3}
%TODO changed the order of the labels to match figure 2 and 3.
% if using array.sty, it might be a good idea to tweak the value of
% \extrarowheight as needed to properly center the text within the cells
